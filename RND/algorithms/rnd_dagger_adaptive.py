...
#     def quantile(self, quantile_level: float = 0.9):
#         # Returns: (lambda_value, min_lambda, max_lambda)
        
#         dataloader = self.dataset.get_dataloader(batch_size=self.batch_size, shuffle=False)

#         if dataloader is None:
#             print("No buffer found, using const lambda threshold")
#             return self.lambda_threshold, self.lambda_threshold, self.lambda_threshold
...
#         if len(scores) == 0:
#             print("no scores calculated with buffer data, using const lambda theshold")
#             return self.lambda_threshold, self.lambda_threshold, self.lambda_threshold

#         scores_tensor = torch.cat(scores, dim=0)
#         lambda_value = torch.quantile(scores_tensor, quantile_level).item()
#         min_lambda = torch.min(scores_tensor).item()
#         max_lambda = torch.max(scores_tensor).item()
#         self.lambda_threshold = lambda_value
#         print(f"lambda value is : {lambda_value}")
#         return lambda_value, min_lambda, max_lambda
...
#                     # current_step = int(self.global_step /500) + 1
#                     # context_states = torch.cat([states, current_step], dim=1) # concat T (26 dim) 
